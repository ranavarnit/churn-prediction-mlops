...
class ChurnPredictor:
    ...
    def train_and_evaluate(self, X, y):
        """
        Train multiple models with hyperparameter tuning and track with MLflow
        """
        ...
        for name, config in models_config.items():
            with mlflow.start_run(run_name=f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
                ...
                if name == 'XGBoost':
                    mlflow.xgboost.log_model(grid_search.best_estimator_, name)
                else:
        ...
        if hasattr(self.best_model, 'feature_importances_'):
            self.feature_importance = dict(zip(X.columns, 
        ...
        self.metrics = {
            ...
            'test_auc': best_auc,
            'all_models': {k: {'auc': v['auc'], 'f1': v['f1']} 
